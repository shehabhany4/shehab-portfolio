...
const Skills = () => {
  ...
  const skillsCategories = [
    ...
    },
    {
      ...
      size: "full",
      skills: [
        { name: language === "ar" ? "العمل الجماعي" : "Teamwork",        icon: <FaUsers />, color: "#ffb300" },
        { name: language === "ar" ? "التواصل"       : "Communication",   text: "COM" },
        { name: language === "ar" ? "حل المشكلات"   : "Problem Solving", text: "PS"  },
        { name: language === "ar" ? "إدارة الوقت"   : "Time Management", text: "TM"  },
        { name: language === "ar" ? "التعلم الذاتي" : "Self-Learning",   text: "SL"  },
        { name: language === "ar" ? "التعاون"        : "Collaboration",  text: "CLB" },
      ],
    },
  ];
    ...
    <section id="skills" className="sk-section" aria-label="Skills and Expertise">
      ...
      <div className="sk-container">
        <header className="sk-header">
          ...
          <p className="sk-subtitle">{t.subtitle}</p>
        </header>
            ...
            >
              ...
              <div className="sk-panel__header">
                ...
                <div className="sk-panel__title-wrap">
                  ...
                  <span className="sk-panel__count">{cat.skills.length} {t.skills}</span>
                </div>
              ...
              <div className="sk-panel__chips">
                {cat.skills.map((skill, i) => (
                  <span
                    key={i}
                    className="sk-chip"
                    style={{ "--chip-color": skill.color || cat.accentColor, "--chip-delay": `${i * 50}ms` }}
                  >
                    {skill.icon && <span className="sk-chip__icon">{skill.icon}</span>}
                    {skill.text && <span className="sk-chip__badge">{skill.text}</span>}
                    <span className="sk-chip__name">{skill.name}</span>
                  </span>
                ))}
              </div>
              <span className="sk-panel__num">{String(index + 1).padStart(2, "0")}</span>
            </article>
          ))}
        </div>
      </div>
